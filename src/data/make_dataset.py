...
@click.command()
@click.argument('class_sample_size', default=100000, type=click.INT)
@click.argument('seed', default=42, type=click.INT)
def main(class_sample_size, seed):
    ...
    for c, class_df in df.groupby('target'):
        logger.info(
            f'generating {class_sample_size} samples for class: {c}')
    ...
    df = df.rename(columns={df.columns[-1]: 'target'})
    df.target = df.target.astype(int)
    logger.info('saving processed data to mitbih_test.csv')
    output_df.to_csv(
